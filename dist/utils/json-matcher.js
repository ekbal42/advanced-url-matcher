...
function jsonMatch(pattern, target) {
    if (pattern === target)
        return true;
    ...
    if (typeof pattern === "string" && pattern.includes(",")) {
        ...
        if (Array.isArray(target)) {
            return target.every((tItem) => allowedValues.some((allowed) => jsonMatch(allowed, tItem)));
        }
        return allowedValues.some((allowed) => {
            if (jsonMatch(allowed, target))
                return true;
    ...
    if (pattern &&
        ...
        for (const key of keys) {
            if (!Object.prototype.hasOwnProperty.call(target, key))
                return false;
            if (!jsonMatch(pattern[key], target[key]))
                return false;
        }
        return true;
    }
    if (Array.isArray(pattern) && Array.isArray(target)) {
        ...
        for (let i = 0; i < pattern.length; i++) {
            if (!jsonMatch(pattern[i], target[i]))
                return false;
        }
        return true;
    }
